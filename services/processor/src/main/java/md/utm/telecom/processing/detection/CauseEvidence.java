...
public final class CauseEvidence {
    ...
    public CauseEvidence(DetectionPolicy policy) { this.policy = policy; }
    ...
    public Explanation explain(JsonNode feature, VoiceSetupRule.Evaluation evaluation, List<JsonNode> receipts) {
        ...
        if (!evaluation.breached() || service == null || ims == null || sip == null || cpu == null
                ...
                || !nearBaseline(kpi(feature, "rrcSrPct"), policy.voice("imsCapacityAccessSrDropPpAtMost"))
                || !nearBaseline(kpi(feature, "bearerSrPct"), policy.voice("imsCapacityAccessSrDropPpAtMost"))
                || ims.path("metrics").path("cpuPct").decimalValue()
                        .compareTo(cpu.path("observed").decimalValue()) != 0) return fallback;
        if (cpu.path("observed").decimalValue().compareTo(policy.voice("imsCapacityCpuPctAtLeast")) < 0
                || sip.path("observed").decimalValue().compareTo(policy.voice("imsCapacitySip503CountAtLeast")) < 0)
            return fallback;
    ...
    private static boolean nearBaseline(JsonNode value, BigDecimal allowedDrop) {
        return value != null && value.path("observed").isNumber() && value.path("baseline").isNumber()
                && value.path("observed").decimalValue().compareTo(
                        value.path("baseline").decimalValue().subtract(allowedDrop)) >= 0;
    }
}
