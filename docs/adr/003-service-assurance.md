...
Integration note: the Day 01 inventory and acceptance record below are historical.
The current stack uses Apache Kafka 3.9.1 with a separate `apache-kafka-data`
volume, blank example passwords, fixed database/role names, a shared fresh/existing
volume provisioner, and runnable generator/processor containers. Follow
[the local development runbook](../runbooks/local-dev.md) for current setup.
